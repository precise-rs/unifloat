...
impl MpfrBounds {
    const fn for_precision_binary(precision_bits: usize) -> Self {
        Self {
            ...
            limb_parts: (precision_bits - 1) / gmp::NUMB_BITS as usize + 1,
        }
    }
    ...
    fn for_precision_decimal(precision_decimal: usize) -> Self {
        let precision_bits =
            (precision_decimal as f32 * core::f32::consts::LOG10_2).ceil() as usize;
...
type UniMpfrLimb1Prec1 = UniFloat<
    {
        UniFloatChoice::Mpfr {
            bounds: MpfrBounds {
                limb_parts: 1,
...
type UniMpfrLimb2PrecAll = UniFloat<
    {
        UniFloatChoice::Mpfr {
            bounds: MpfrBounds {
                limb_parts: 2,
...
impl<const BASE: UniFloatBoundsBase> UniFloatBounds<BASE> {
    ...
    pub const fn covers(&self, other: &Self) -> bool {
        self.precision >= other.precision
            && self.min_exponent <= other.min_exponent
            && self.max_exponent >= other.max_exponent
    }
}

const F32_BOUNDS_BINARY: UniFloatBounds<{ UniFloatBoundsBase::BINARY }> =
    UniFloatBounds::<{ UniFloatBoundsBase::BINARY }> {
        precision: f32::MANTISSA_DIGITS as usize,
        min_exponent: f32::MIN_EXP as isize,
        max_exponent: f32::MAX_EXP as isize,
    };
const F32_BOUNDS_DECIMAL: UniFloatBounds<{ UniFloatBoundsBase::DECIMAL }> =
    UniFloatBounds::<{ UniFloatBoundsBase::DECIMAL }> {
        precision: f32::DIGITS as usize,
        min_exponent: f32::MIN_10_EXP as isize,
        max_exponent: f32::MAX_10_EXP as isize,
    };
const F64_BOUNDS_BINARY: UniFloatBounds<{ UniFloatBoundsBase::BINARY }> =
    UniFloatBounds::<{ UniFloatBoundsBase::BINARY }> {
        precision: f64::MANTISSA_DIGITS as usize,
        min_exponent: f64::MIN_EXP as isize,
        max_exponent: f64::MAX_EXP as isize,
    };
const F64_BOUNDS_DECIMAL: UniFloatBounds<{ UniFloatBoundsBase::DECIMAL }> =
    UniFloatBounds::<{ UniFloatBoundsBase::DECIMAL }> {
        precision: f64::DIGITS as usize,
        min_exponent: f64::MIN_10_EXP as isize,
        max_exponent: f64::MAX_10_EXP as isize,
    };
const TWOFLOAT_BOUNDS_BINARY: UniFloatBounds<{ UniFloatBoundsBase::BINARY }> =
    UniFloatBounds::<{ UniFloatBoundsBase::BINARY }> {
        precision: 2 * f64::MANTISSA_DIGITS as usize,
        min_exponent: f64::MIN_EXP as isize,
        max_exponent: f64::MAX_EXP as isize,
    };
const TWOFLOAT_BOUNDS_DECIMAL: UniFloatBounds<{ UniFloatBoundsBase::DECIMAL }> =
    UniFloatBounds::<{ UniFloatBoundsBase::DECIMAL }> {
...
impl UniFloatChoice {
    ...
    /// Also, beware that without `compact` feature, F32-based UniFloat takes as much space
...
pub type MpfrLimbPart = mem::MaybeUninit<gmp::limb_t>;
#[allow(dead_code)] // not used with compact feature.
...
pub struct UniFloat<const C: UniFloatChoice>
where
    ...
    [MpfrLimbPart; mpfr_limb_parts_length(C)]: Sized,
{
    ...
    /// A pointer to UniFloat instance itself. Used for extra .sanitize() check.
    /// Beneficial for testing the right usage of the .sanitize() and <<= API even without UniFloatChoice::Mpfr.
    unifloat_self: *const UniFloat<C>,

    #[cfg(debug_assertions)]
    /// A (limited) safeguard for confirming that we've applied .mutate() on the same instance only once - until it's cleared with .sanitize() or <<=.
...
#[allow(dead_code)]
const DUMMY_MPFR_LIMB_PTR: ptr::NonNull<gmp::limb_t> =
    unsafe { core::ptr::NonNull::new_unchecked(&DUMMY_MPFR_LIMB as *const _ as *mut gmp::limb_t) };
/// Never leaked to the user.
/// Based on gmp_mpfr_sys::MPFR_DECL_INIT
#[allow(dead_code)]
const INITIAL_MPFR_EXP: mpfr::exp_t = 1 - mpfr::exp_t::max_value();

impl<const C: UniFloatChoice> Default for UniFloat<C>
where
    [f32; f32_parts_length(C)]: Sized,
    [f64; f64_parts_length(C)]: Sized,
    [twofloat::TwoFloat; twofloat_parts_length(C)]: Sized,
...
impl<const C: UniFloatChoice> UniFloat<C>
where
    [f32; f32_parts_length(C)]: Sized,
    [f64; f64_parts_length(C)]: Sized,
    [twofloat::TwoFloat; twofloat_parts_length(C)]: Sized,
    ...
    [MpfrLimbPart; mpfr_limb_parts_length(C)]: Sized,
{
    /// Not-a-Number. Unsanitized!
    /// Do NOT use in expressions directly (because it's unsanitized). Instead, first assign/copy it to a (mutable)
    /// variable, and sanitize. Assign either with with <<=, or with = and call .sanitize().
    /// The same applies to any other constants/static (immutable) UniFloat values.
    ...
    fn mpfr_limps_ptr(&self) -> ptr::NonNull<gmp::limb_t> {
        unsafe { ptr::NonNull::new_unchecked(self.mpfr_limbs[..].as_ptr() as *mut gmp::limb_t) }
    }
    ...
    fn assert_copy_fixed(&self) {
        #[cfg(debug_assertions)]
        {
            assert!(self.unifloat_self == self,
                "Must call .sanitize() first, or assign with <<= instead of =. (unifloat_self hasn't been fixed.)");
            assert!(self.used_as_operand_mutated,
                 "Must call .sanitize() first, or assign with <<= instead of =. (used_as_mut_ref_operand hasn't been cleared.)" );
        }
        #[cfg(not(feature = "compact"))]
        assert!(
            ...
            "MPFR fields indicate that the instance was copied/moved without having called .sanitize() afterwards, or it was assign to with = instead of <<=. However, unifloat_self guard didn't catch this. Please report this to UniFloat along with how to reproduce it in debug mode.");
    }
    ...
    fn assert_copy_not_fixed(&self) {
        #[cfg(debug_assertions)]
        assert!(self.unifloat_self != self,
            "Have already called .sanitize(), or assigned with <<= instead of =. Do not call .sanitize() again.");
        #[cfg(not(feature = "compact"))]
        assert!(
            if let UniFloatChoice::Mpfr { .. } = C {
                self.mpfr_fixeds[0].d != self.mpfr_limps_ptr()
            ...
            "MPFR fields indicate that the code had already called .sanitize(), or assigned with <<= instead of =. However, unifloat_self guard didn't catch this. Please report this to UniFloat along with how to reproduce it in debug mode.");
    }

    /// Call this exactly once after a UniFloat value is
    /// assigned to, received as a
    /// parameter, received from a result of a called function or an expression, copied to or from a
    /// struct/tuple/array.
    /// However, do not call this if the value was assigned to with <<=
    /// operator. Do not call this on shared or mutable references.
    #[inline]
    pub fn sanitize(&mut self) -> &mut Self {
        // @TODO Put it somewhere frequent. Assert that the version is odd exactly when using `compact` feature.
        if false {
            panic!("Major version: {}", pkg_version::pkg_version_major!());
        }
...
// @TODO
// impl num_traits::float::Float for UniFloat<C>
// classify(), is_sign_positive()... - like https://doc.rust-lang.org/std/primitive.f32.html#method.classify
// --- https://doc.rust-lang.org/core/num/enum.FpCategory.html

impl<const C: UniFloatChoice> ops::ShlAssign for UniFloat<C>
where
    [f32; f32_parts_length(C)]: Sized,
    [f64; f64_parts_length(C)]: Sized,
    [twofloat::TwoFloat; twofloat_parts_length(C)]: Sized,
...
impl<const C: UniFloatChoice> ops::ShlAssign<&Self> for UniFloat<C>
where
    [f32; f32_parts_length(C)]: Sized,
    [f64; f64_parts_length(C)]: Sized,
    [twofloat::TwoFloat; twofloat_parts_length(C)]: Sized,
